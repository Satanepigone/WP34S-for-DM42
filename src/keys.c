...
{
	static const unsigned char linear_key_map[ 7 * 6 - 1 + 6 ] = {
		 0,  1,  2,  3,  4,  5,   // K00 - K05
		 6,  7,  8,  9, 10, 11,   // K10 - K15
		 12, 13, 14, 15, 16,  0,   // K20 - K24
		17, 18, 19, 20, 21,  0,   // K30 - K34
		22, 23, 24, 25, 26,  0,   // K40 - K44
		27, 28, 29, 30, 31,  0,   // K50 - K54
		32, 33, 34, 35, 36,       // K60 - K64
		37, 37, 37, 37, 37, 37       // K_F, G, H, ARROW, CMPLX, NOP
	};
...
static int keycode_to_linear(const keycode c)
{
	static const unsigned char linear_key_map[ 7 * 6 - 1 + 6 ] = {
		...
		29, 30, 31, 32, 33,       // K60 - K64
		34, 34, 34,  4,  5, 34,        // K_F, G, H, ARROW, CMPLX, NOP
	};
	return linear_key_map[c];
}
#else
 ...
 */
// Decision to make here about whether the top row can be used as shortcuts.
// Also, which other keys can / can't be used as shortcuts?
// Registers I, j, K, L will move with the new alpha layout.
...
unsigned int keycode_to_digit_or_register(const keycode c)
{
  static const unsigned char map[] = {
    ...
    // K20 - K24
    NO_SHORT | NO_REG, NO_SHORT | NO_REG, NO_SHORT | NO_REG, NO_SHORT | NO_REG, NO_SHORT | NO_REG,
    // K30 - K34
    NO_REG, 7, 8, 9, NO_REG,
    // K40 - K44
    NO_REG, NO_SHORT | regT_idx, 5, 6, NO_REG,
    // K50 - K54
...
#ifdef DM42
      { K04, { CATALOGUE_CONV,      CATALOGUE_NONE,      CATALOGUE_CONV          } },
#else
...
static int keycode_to_alpha(const keycode c, unsigned int shift)
{
	static const unsigned char alphamap[][6] = {
		...
		{ 'J',  0257, 0000, 0000, 'j',  0000,  },  // K13
		{ 'K',  0000, 'K',  0000, 'k',  0251,  },  // K14
		{ 'L',  0000, 0212, 0000, 'l',  0252,  },  // K15

		{ 0000, 0000, 'H',  0000, 0000, 0246,  },  // K20 ENTER
		{ 'M',  0000, 0253, 0027, 'm',  0253,  },  // K21
		{ 'N',  0010, 'N',  0225, 'n',  0254,  },  // K22
		{ 'O',  0000, 0227, 0257, 'o',  0267,  },  // K23
		{ 0000, 0000, 0000, 0000, 0000, 0000   },  // K24 <-

		{ 0000, 0000, 0000, 0000, 0000, 0000,  },  // K30
		{ 'P',  '7',  0217, '&',  'p',  0257,  },  // K31
		{ 'Q',  '8',  0000, '|',  'q',  0000,  },  // K32
		{ 'R',  '9',  'R',  0000, 'r',  0260,  },  // K33
		{ 'S',  '/',  0221, 0000, 's',  0261,  },  // K34

		{ 0000, 0000, 0000, 0000, 0000, 0000,  },  // K40
		{ 'T',  '4',  'T',  0000, 't',  0262,  },  // K41
		{ 'U',  '5',  0000, 0000, 'u',  0000,  },  // K42
		{ 'V',  '6',  0000, 0000, 'v',  0000,  },  // K43
		{ 'W', 0034,  0000, 0000, 'w',  0000,  },  // K44

		{ 0000, 0000, 0000, 0000, 0000, 0000,  },  // K50
		{ 'X',  '1',  0215, 0000, 'x',  0255,  },  // K51
		{ 'Y',  '2',  'Y',  0000, 'y',  0263,  },  // K52
		{ 'Z',  '3',  'Z',  0000, 'z',  0245,  },  // K53
		{ '-',  '(',  ')',  0000, 0000, 0000,  },  // K54

		{ 0000, 0000, 0000, 0000, 0000, 0000,  },  // K60
		{ ':',  '0',  0207, 0000, ';',  0247,  },  // K61
		{ '.',  '.',  0226, 0000, '.',  0266,  },  // K62
		{ '?',  0000, 0041, 0000, 0000, 0000,  },  // K63
		{ ' ',  '+',  '=',  0000, 0000, 0000,  },  // K64
	...
	return alphamap[keycode_to_linear_for_alpha(c)][shift];
}
#else
/*
...
#else
	  if (UState.intm && shift == SHIFT_F) {
			return check_f_key(2, op_map2[lc]);
		}
		break;
#endif
	case K20:				// Alpha
...
#ifdef DM42
	case K31:
	  if ((shift == SHIFT_G) && UState.intm && intltr(13)) {// Log - so D key
	    return OP_SPEC | OP_D;
	  }
	  break;
	case K30:
	  if ((shift == SHIFT_G) && UState.intm && intltr(14)) {// Ln - so E key
	    return OP_SPEC | OP_E;
	  }
	  break;
#endif
...
#ifdef DM42
		{ OP_NIL | OP_NOP,	OP_NIL | OP_NOP,    OP_NIL | OP_NOP,    OP_NIL | OP_NOP    }
#endif
	};
...
#ifdef DM42
	if ( ! shorthand && ! State2.dot && ! State2.shuffle ) {
	  switch (n) {
	  case regX_idx:
	    return arg_digit(1);
	  case regY_idx:
...
static int process_test(const keycode c) {
	...
	State2.test = TST_NONE;
	State2.cmplx = 0;
	if (n == regX_idx) n = 1; //don't want x-reg
...
static int process_labellist(const keycode c) {
	...
	case K_UP | (SHIFT_F << 8):		// Find first label of previous program
		pc = advance_to_previous_label(advance_to_previous_label(pc, 1), 1);
		goto next;

	case K_DOWN | (SHIFT_F << 8):		// Find next program
		pc = advance_to_next_label(pc, 0, 1);
	case K_DOWN:				// Find next label
	next:
		State2.digval = advance_to_next_label(pc, 1, 0);
		return STATE_UNFINISHED;

	case K_UP:				// Find previous label
