...
### Recent changes (new users start with "Description" further down the page):
  * This is the Romberg branch. The old Romberg integration routine (from build 3892) is available in the Integrate menu (shift 8) as the command ROM[integral]. Everything else is the same as the main Master branch.
